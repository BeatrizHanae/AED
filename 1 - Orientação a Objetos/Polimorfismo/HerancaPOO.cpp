...
class veiculo{
	...
	// No Encapsulamento os atributos são privados, ou seja,
	//só podem ser acessados a partir de métodos da classe que sejam públicos.  
	...
	// É importante deixar os métodos de uma classe como públicos, para que
	//seja possível acessar e modificar atributos de fora da classe.
	public:  
	
	int n_passageiros;	
	
	void set_marca (string word) {marca = word;}   // Métodos setters e getters : 
	string get_marca () {return marca;}			//O primeiro serve para atribuir valores e o segundo para visualizá-los.
...
class caminhonete : public veiculo{  // Repare que a herança ocorreu de forma pública.
	...
	//   ricos da classe mãe, também possui especificidades. 
	
	int carga;
	
	public:
	
	int get_passageiros () {cout << "Nao transporta passageiros\n"; } // reutilizamos o metodo da classe mãe,
	//porém alterando como se caminhonetes não transportassem passageiros, mostrado uma implementação do Polimorfismo.
		...
		switch(menu){
			...
			case 2:{
				...
				if(tipo_veiculo == "caminhonete"){
					...
					cout << "Digite marca, alimentacao, numero de rodas e a carga do veiculo, separados por enter:" << endl;
					cin >> auxiliar[0] >> auxiliar[1] >> auxiliar2[0] >> auxiliar2[1];
					...
					cout << v1.get_passageiros() << endl;//Ao mostrar os valores cadastrados
					//o get_passageiros de v1 retorna outro tipo de saida devido a alteração
					//do metodo dentro da classe caminhonete, caracterizando o Polimorfismo
