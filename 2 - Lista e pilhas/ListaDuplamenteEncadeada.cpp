...
// ela implementa uma Lista duplamente encadeada. E recomendavel que o aluno repita o mesmo exercicio
// implementando Listas duplamente encadeadas com ponteiros, como foi feito para a Lista circular
...
void Lista::inserirElementos(){
	int n;
		do{
		cout << "Como voc? deseja inserir o elemento?" << endl;
		cout << "1. Lista final da Lista" << endl;
		cout << "2. Em uma posi??o conhecida" << endl;
		cin >> n;
		}while(n<0 || n>2);
...
void Lista::buscarElementos(int Elemento){
	...
	for (it = lista.begin(); it!=lista.end(); it++){
		if (*it == Elemento){ 
			cout << "O elemento esta presente na " << i << "e posicao" << endl; 
			}
		i++;
	}
}
...
void Lista::removerElementos(){
		int n;
		do{
		cout << "Como voc? deseja remover o elemento?" << endl;
		cout << "1. Por valor (primeira ocorr?ncia do valor)" << endl;
		cout << "2. Pela Posi??o" << endl;
		cin >> n;
		}while(n<0 || n>2);
	...
	switch(n){
		case 1:
			cout << "Entre com o Valor do Elemento que voc? deseja remover" << endl;
			cin >> valor;
			if (primeiraOcorrencia(valor)>=0){
				removerElemento(primeiraOcorrencia(valor));
			}
			else{
				cout << "Erro! Elemento n?o encontrado na lista" << endl;
			}
			break;
		case 2:
